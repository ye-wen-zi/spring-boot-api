...
public class AuthService {
    ...
    @Value("${jwt.accessTokennExpiration:900000}")
    private long ACCESS_TOKEN_EXPIRATION;

    @Value("${jwt.refreshTokennExpiration:86400000}") // 1 day -> ms
        ...
        if (passwordEncoder.matches(authInfo.password(), user.getPassword())) {
            ...
            return ResponseEntity
                    ...
                    .header(HttpHeaders.SET_COOKIE,
                            createCookies(refeshToken, REFRESH_TOKEN_EXPIRATION / 1000).toString())
    ...
    public ResponseEntity<?> refresh(String refreshToken) {
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.SET_COOKIE,
                        this.createCookies(newRefreshToken, REFRESH_TOKEN_EXPIRATION / 1000).toString())
